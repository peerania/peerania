...
namespace eosio
{
void peerania::registeracc(account_name owner, std::string displayname, std::string ipfsprofile)
{
    require_auth(owner);
    eosio_assert(_accounts.find(owner) == _accounts.end(), "Account already exists");
    eosio_assert(displayname.length() > MIN_DISPLAYNAME_LEN, "The displayname too short.");
...
void peerania::set_account_parameter(account_name owner, const prop_key_value &keyvalue)
{
    ...
    {
        uptable.modify(itr, _self, [&](auto &prop) {
            prop.kv.value = keyvalue.value;
        });
    }
}
...
void peerania::set_account_ipfs_profile(account_name owner, const std::string &ipfsprofile)
{
    auto itr = _accounts.find(owner);
    eosio_assert(itr != _accounts.end(), "Account not found");
    _accounts.modify(itr, _self, [&](auto &account) {
        account.ipfsprofile = ipfsprofile;
    });
}
...
void peerania::setdispname(account_name owner, std::string displayname)
{
    ...
    _accounts.modify(itraccs, _self, [&](auto &acc) {
        acc.displayname = displayname;
    });
    add_displayname_to_map(owner, displayname);
}

void peerania::add_displayname_to_map(account_name owner, const std::string &displayname)
{
...
void peerania::remove_display_name_from_map(account_name owner, const std::string &displayname)
{
...
void peerania::require_for_an_account(account_name owner){
    eosio_assert(_accounts.find(owner) != _accounts.end(), "Account not registered");
}

}; // namespace eosio
EOSIO_ABI(eosio::peerania, (adeleteacc)(registeracc)(setaccparam)(asetaccparam)(asetipfspro)(setipfspro)(setdispname))
