...
class peerania : public eosio::contract {
 public:
  ...
  // Register new user
  [[eosio::action]] 
  void registeracc(account_name owner,
                                      std::string display_name,
                                      std::string ipfs_profile);

  // Set(or add) property to account
  [[eosio::action]] 
  void setaccstrprp(account_name owner, uint8_t key,
                                      std::string value);

  // Set(or add) property to account
  [[eosio::action]] 
  void setaccintprp(account_name owner, uint8_t key,
                                      int32_t value);

  // Set user profile (IPFS link)
  [[eosio::action]] 
  void setipfspro(account_name owner,
                                    std::string ipfs_profile);

  // Set user display name
  [[eosio::action]] 
  void setdispname(account_name owner,
                                     std::string display_name);
  ...
  // passed answer_id
  [[eosio::action]] 
  void postcomment(account_name user, uint64_t question_id,
                                     uint16_t answer_id,
  ...
  // Delete answer
  [[eosio::action]] 
  void delanswer(account_name user, uint64_t question_id,
                                   uint16_t answer_id);

  // Delete comment
  [[eosio::action]] 
  void delcomment(account_name user, uint64_t question_id,
  ...
  // Modify answer
  [[eosio::action]] 
  void modanswer(account_name user, uint64_t question_id,
                                   uint16_t answer_id,
                                   const std::string &ipfs_link);

  // Modify comment
  [[eosio::action]] 
  void modcomment(account_name user, uint64_t question_id,
                                    uint16_t answer_id, uint16_t comment_id,
                                    const std::string &ipfs_link);

  // Upvote question/answer
  [[eosio::action]] 
  void upvote(account_name user, uint64_t question_id,
                                uint16_t answer_id);

  // Downvote question/answer
  [[eosio::action]] 
  void downvote(account_name user, uint64_t question_id,
                                  uint16_t answer_id);
  ...
  [[eosio::action]] 
  void votedelete(account_name user, uint64_t question_id,
                                    uint16_t answer_id, uint16_t comment_id);
  ...
  // question(question_id)->answer(answer_id)->comment(by comment_id)
  [[eosio::action]] 
  void votemoderate(account_name user, uint64_t question_id,
                                      uint16_t answer_id, uint16_t comment_id);
