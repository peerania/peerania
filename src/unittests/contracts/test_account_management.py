...
class AccountManagementTests(peeraniatest.PeeraniaTest):
    ...
    def test_register_user(self):
        ...
        self.assertTrue(compare(['#ignoreorder',
                                 self.get_expected_account_body(alice)], t))
    ...
    def test_user_property(self):
        ...
        for i in range(3):
            ...
            e[1]['integer_properties'].append({'key': i+5, 'value': i*i*i})
            regInt(bob, i+1, i*i)
    ...
    def test_management_for_non_existent_account_failed(self):
        begin("Testing account changing IPFS profile and display_name for non-existing account", True)
        alice = self.get_non_registered_alice()
    ...
    def test_management_another_owner_failed(self):
        begin('Testing account changing IPFS profile and display_name with another account', True)
        alice = self.register_alice_account()
        bob = self.register_bob_account()
        self.failed_action('setipfspro',  {
