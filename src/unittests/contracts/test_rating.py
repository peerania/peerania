...
class ForumRatingRewardsTests(peeraniatest.PeeraniaTest):
    ...
    def test_delete_own_item_reward(self):
        ...
        self.action('delquestion', {'user': 'bob', 'question_id': self.var['bq']}, bob, "Delete Bob question")
    ...
    def test_delete_question_reward(self):
        ...
        self.assertTrue(compare(self.account_e, self.table(
            'account', 'allaccounts'), self.var, ignore_excess=True))
        (bob_old_rt, alice_old_rt, carol_old_rt) = (self.var['bob_rating'], self.var['alice_rating'], self.var['carol_rating'])
        self.action('upvote', {'user': 'alice', 'question_id': self.var['aq'], 'answer_id': self.var['aq_ba']}, alice, "Alice upvote bob answer")
        self.action('mrkascorrect', {'user': 'alice', 'question_id': self.var['aq'], 'answer_id': self.var['aq_ba']}, alice, "Alice mark bob answer as correct")
        self.action('upvote', {'user': 'bob', 'question_id': self.var['aq'], 'answer_id': 0}, bob, "Bob upvote alice question")
        self.action('upvote', {'user': 'alice', 'question_id': self.var['aq'], 'answer_id': self.var['aq_ca']}, alice, "Alice upvote carol answer")
        self.action('upvote', {'user': 'carol', 'question_id': self.var['aq'], 'answer_id': 0}, carol, "Carol upvote alice question")
        self.action('upvote', {'user': 'bob', 'question_id': self.var['aq'], 'answer_id': self.var['aq_ca']}, bob, "Bob upvote carol answer")
        self.action('upvote', {'user': 'carol', 'question_id': self.var['aq'], 'answer_id': self.var['aq_ba']}, carol, "Carol upvote bob answer")
        self.action('upvote', {'user': 'dan', 'question_id': self.var['aq'], 'answer_id': 0}, dan, "Dan upvote alice question")
        self.action('upvote', {'user': 'dan', 'question_id': self.var['aq'], 'answer_id': self.var['aq_ba']}, dan, "Dan upvote bob answer")
        self.action('upvote', {'user': 'dan', 'question_id': self.var['aq'], 'answer_id': self.var['aq_ca']}, dan, "Dan upvote carol answer")
        self.var['bob_rating'] += 3 * self.defs['ANSWER_UPVOTED_REWARD'] + self.defs['ANSWER_ACCEPTED_AS_CORRECT_REWARD']
        self.var['alice_rating'] += 3 * self.defs['QUESTION_UPVOTED_REWARD'] + self.defs['ACCEPT_ANSWER_AS_CORRECT_REWARD']
        self.var['carol_rating'] += 3 * self.defs['ANSWER_UPVOTED_REWARD']
        self._verify_acc()
        self.action('votedelete', {'user': 'ted', 'question_id': self.var['aq'], 'answer_id': 0, 'comment_id': 0}, ted, "Ted delete alice question")
        self.var['ted_mdp'] -= 1
        (self.var['bob_rating'], self.var['alice_rating'], self.var['carol_rating']) = (bob_old_rt, alice_old_rt, carol_old_rt)
    ...
    def test_delete_answer_reward(self):
        ...
        self.var['alice_rating'] += 3 * self.defs['QUESTION_UPVOTED_REWARD'] + self.defs['ACCEPT_ANSWER_AS_CORRECT_REWARD']
        self._verify_acc()
        self.action('votedelete', {'user': 'ted', 'question_id': self.var['aq'], 'answer_id': self.var['aq_ba'], 'comment_id': 0}, ted, "Ted delete alice question->bob answer")
        self.var['ted_mdp'] -= 1
        self.var['alice_rating'] -= self.defs['ACCEPT_ANSWER_AS_CORRECT_REWARD']
        self.var['bob_rating'] = bob_old_rt + self.defs['ANSWER_DELETED_REWARD']
        self._verify_acc()
        end()
