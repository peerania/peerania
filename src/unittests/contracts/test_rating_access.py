...
class ForumRatingRewardsTests(peeraniatest.PeeraniaTest):
    ...
    def test_post_question(self):
        ...
        self.action('setaccrtmpc', {
                    'user': 'alice', 'rating': defs['POST_QUESTION_ALLOWED'] - 1, 'moderation_points': 1}, alice, "Reduce alice rating for 1")
    ...
    def test_post_answer(self):
        ...
        self.action('setaccrtmpc', {'user': 'alice', 'rating': defs['POST_ANSWER_OWN_ALLOWED'] - 1,
                                    'moderation_points': 1}, alice, "Set alice rating to { POST_ANSWR_OWN_ALLOWED - 1 }")
        ...
        self.wait()  # why???
        self.action('setaccrtmpc', {'user': 'alice', 'rating': defs['POST_ANSWER_OWN_ALLOWED'],
                                    'moderation_points': 1}, alice, "Set alice rating to { POST_ANSWR_OWN_ALLOWED }")
    ...
    def test_post_comment(self):
        ...
        carol = self.register_carol_account(
            defs['POST_COMMENT_ALLOWED'] - 1, 1)
        (e, var) = self._create_simple_hierarchy(alice, bob)
        self.action('setaccrtmpc', {'user': 'alice', 'rating': defs['POST_COMMENT_OWN_ALLOWED'] - 1,
                                    'moderation_points': 1}, alice, "Set alice rating to { POST_COMMENT_OWN_ALLOWED - 1 }")
        self.action('setaccrtmpc', {'user': 'bob', 'rating': defs['POST_COMMENT_OWN_ALLOWED'] - 1,
                                    'moderation_points': 1}, bob, "Set bob rating to { POST_COMMENT_OWN_ALLOWED - 1 }")
        ...
        self.action('setaccrtmpc', {'user': 'alice', 'rating': defs['POST_COMMENT_OWN_ALLOWED'],
                                    'moderation_points': 1}, alice, "Set alice rating to { POST_COMMENT_OWN_ALLOWED }")
        self.action('setaccrtmpc', {'user': 'bob', 'rating': defs['POST_COMMENT_OWN_ALLOWED'],
                                    'moderation_points': 1}, bob, "Set bob rating to { POST_COMMENT_OWN_ALLOWED }")
        self.action('setaccrtmpc', {'user': 'carol', 'rating': defs['POST_COMMENT_ALLOWED'],
                                    'moderation_points': 1}, carol, "Set carol rating to { POST_COMMENT_ALLOWED }")
    ...
    def test_upvote(self):
        ...
        self.action('setaccrtmpc', {
                    'user': 'carol', 'rating': defs['UPVOTE_ALLOWED'], 'moderation_points': 1}, carol, "Set carol rating to { UPVOTE_ALLOWED }")
        self.wait()
        self.action('upvote', {
    ...
    def test_downvote(self):
        ...
        self.action('setaccrtmpc', {'user': 'carol', 'rating': defs['DOWNVOTE_ALLOWED'],
                                    'moderation_points': 1}, carol, "Set carol rating to { DOWNVOTE_ALLOWED }")
        self.wait()
    ...
    def test_create_tag_or_community(self):
        ...
        carol = self.register_carol_account(
            defs['CREATE_COMMUNITY_ALLOWED'], 10)
        dan = self.register_dan_account(
            defs['CREATE_COMMUNITY_ALLOWED'] - 1, 10)
    ...
    def test_vote_create_tag_or_community(self):
        ...
        alice = self.register_alice_account(
            defs['VOTE_CREATE_TAG_ALLOWED'], 10)
        bob = self.register_bob_account(
            defs['VOTE_CREATE_TAG_ALLOWED'] - 1, 10)
        carol = self.register_carol_account(
            defs['VOTE_CREATE_COMMUNITY_ALLOWED'], 10)
        dan = self.register_dan_account(
            defs['VOTE_CREATE_COMMUNITY_ALLOWED'] - 1, 10)
    ...
    def test_vote_delete_tag_or_community(self):
        ...
        t = self.table('crtagtb', get_tag_scope(1))
        alice = self.register_alice_account(
            defs['VOTE_DELETE_TAG_ALLOWED'], 10)
        bob = self.register_bob_account(
            defs['VOTE_DELETE_TAG_ALLOWED'] - 1, 10)
        carol = self.register_carol_account(
            defs['VOTE_DELETE_COMMUNITY_ALLOWED'], 10)
        dan = self.register_dan_account(
            defs['VOTE_DELETE_COMMUNITY_ALLOWED'] - 1, 10)
