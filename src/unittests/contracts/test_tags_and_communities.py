...
class TagsAndCommunitiesTests(peeraniatest.PeeraniaTest):
    ...
    def test_non_existing_tag_community_failed(self):
        ...
        self.failed_action('postquestion', {'user': 'alice', 'community_id': 3, 'tags': [1], 'title': 'Title alice question', 'ipfs_link': 'AQ'}, alice,
                           'Alice attempt to ask question with non-existing community - 3', 'assert')
        self.action('postquestion', {'user': 'alice', 'community_id': 1, 'tags': [1], 'title': 'Title alice question', 'ipfs_link': 'AQ'}, alice,
                    'Alice ask question')
        var = {}
        self.assertTrue(compare([{'id': '#var aq',
        ...
        self.failed_action('postquestion', {'user': 'alice', 'question_id': var['aq'],  'community_id': 3, 'tags': [1], 'title': 'Title alice question', 'ipfs_link': 'AQ'}, alice,
                           'Alice attempt to modify own question - set non-existing community - 3', 'assert')
        end()
    ...
    def test_create_community_and_tag(self):
        ...
        for i in range(4):
            if i < 3:
                ...
                self.assertTrue({'id': 6, 'name': 'bob tag', 'ipfs_description': 'BCM', 'popularity': 0} in self.table('tagandcomm', get_tag_scope(1)))
            self.action('vtcrcomm', {'user': voters[i], 'community_id': c[0]['id']}, voters[i], f'{voters[i]} vote create community')
        self.assertTrue(self.table('crtagcomm', get_tag_scope(1)) == [])
        self.assertTrue({'id': 3, 'name': 'alice community',
    ...
    def test_community_and_tag_vote_non_existent(self):
        ...
        self.failed_action('vtcrcomm', {'user': carol, 'community_id': c[0]['id'] - 1}, carol, 'Carol vote create non-existent community', 'assert')
        self.failed_action('vtdelcomm', {'user': carol, 'community_id': c[0]['id'] - 1}, carol, 'Carol vote delete non-existent community', 'assert')
        self.failed_action('vtcrtag', {'user': carol, 'community_id': 1, 'tag_id': t[0]['id'] - 1}, carol, 'Carol vote create non-existent tag', 'assert')
        self.failed_action('vtdeltag', {'user': carol, 'community_id': 3, 'tag_id': t[0]['id'] }, carol, 'Carol vote create non-existent tag', 'assert')
        self.failed_action('vtcrtag', {'user': carol, 'community_id': 1, 'tag_id': t[0]['id'] - 1}, carol, 'Carol vote delete non-existent tag', 'assert')
        self.failed_action('vtdeltag', {'user': carol, 'community_id': 3, 'tag_id': t[0]['id']}, carol, 'Carol vote delete non-existent tag', 'assert')
        end()

    def test_delete_without_downvotes_own(self):
        begin('Test delete own tag and community without downvote', True)
        alice = self.register_alice_account(10000, 10)
        bob = self.register_bob_account(10000, 10)
        carol = self.register_carol_account(10000, 10)
        self.action('crcommunity', {'user': alice, 'name': 'alice community',
    ...
    def test_delete_with_downvotes_own(self):
        ...
        self.action('vtdelcomm', {'user': carol, 'community_id': c[0]['id']}, carol, 'Carol vote delete alice community')
        self.action('vtdeltag', {'user': carol, 'community_id': 1, 'tag_id': t[0]['id']}, carol, 'Carol vote delete bob tag')
        self.action('vtdelcomm', {'user': bob, 'community_id': c[0]['id']}, bob, 'Bob vote delete alice community')
        self.action('vtdeltag', {'user': alice, 'community_id': 1, 'tag_id': t[0]['id']}, alice, 'Alice vote delete bob tag')
        self.action('vtdelcomm', {'user': alice, 'community_id': c[0]['id']}, alice, 'Alice vote delete own community')
        self.action('vtdeltag', {'user': bob, 'community_id': 1, 'tag_id': t[0]['id']}, bob, 'Bob vote delete own tag')
        self.assertTrue(self.table('crtagcomm', 'allcomm') == [])
        self.assertTrue(self.table('crtagcomm', get_tag_scope(1)) == [])
    ...
    def test_moderation_point_limit(self):
        ...
        defs = load_defines('./src/contracts/peerania/communities_and_tags.hpp')
        alice = self.register_alice_account(10000, defs['MODERATION_POINTS_CREATE_COMMUNITY'])
        bob = self.register_bob_account(10000, defs['MODERATION_POINTS_CREATE_TAG'])
        carol = self.register_carol_account(10000, defs['MODERATION_POINTS_CREATE_COMMUNITY'] - 1)
        dan = self.register_dan_account(10000, defs['MODERATION_POINTS_CREATE_TAG'] - 1)
    ...
    def test_upvote_downvote_change_vote(self):
        begin('Test upvote after downvote and downvote after upvote', True)
        alice = self.register_alice_account(10000, 10)
        bob = self.register_bob_account(10000, 10)
        self.action('crcommunity', {'user': 'alice', 'name': 'ACM', 'ipfs_description': 'Alice community description'}, alice,
                    'Alice create community')
        ...
        self.failed_action('vtcrtag', {'user': bob, 'community_id': 1,
                                       'tag_id': t[0]['id']}, bob, 'Bob attempt to vote create tag again', 'assert')
        self.failed_action('vtcrcomm', {
            'user': bob, 'community_id': c[0]['id']}, bob, 'Bob attempt to vote create community again', 'assert')
        self.failed_action('vtdeltag', {'user': bob, 'community_id': 1,
                                        'tag_id': t[0]['id']}, bob, 'Bob attempt to vote delete tag', 'assert')
        self.failed_action('vtdelcomm', {
            'user': bob, 'community_id': c[0]['id']}, bob, 'Bob attempt to vote delete community', 'assert')
        ...
        self.wait()
        self.failed_action('vtdeltag', {'user': carol, 'community_id': 1,
                                        'tag_id': t[0]['id']}, carol, 'Carol vote delete tag again', 'assert')
        self.failed_action('vtdelcomm', {
            'user': carol, 'community_id': c[0]['id']}, carol, 'Carol vote delete community again', 'assert')
        self.failed_action('vtcrtag', {'user': carol, 'community_id': 1,
                                        'tag_id': t[0]['id']}, carol, 'Carol vote create tag', 'assert')
        self.failed_action('vtdelcomm', {
