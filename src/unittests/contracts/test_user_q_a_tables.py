...
class UserQuestionsAndAnswersTablesTests(peeranhatest.peeranhaTest):
    ...
    def test_delete_own_item(self):
        ...
        self.action('delanswer', {
                    'user': 'bob', 'question_id': var['aq'], 'answer_id': var['aq_ba']}, bob, 'Bob delete own answer')
        self.assertTrue(compare([], self.table('usranswers', bob)))
        self.action('delanswer', {
                    'user': 'alice', 'question_id': var['aq'], 'answer_id': var['aq_aa']}, alice, 'Alice delete own answer')
    ...
    def test_delete_question_by_vote(self):
        ...
        self.action('reportforum', {
                    'user': dan, 'question_id': var['aq'], 'answer_id': 0, 'comment_id': 0}, dan, 'Dan report(delete) alice question')
        self.assertTrue(compare([], self.table('usranswers', bob)))
        self.assertTrue(compare([], self.table('usranswers', alice)))
        self.assertTrue(compare([], self.table('usrquestions', alice)))
        end()
    ...
    def test_delete_answer_by_vote(self):
        ...
        self.assertTrue(compare([], self.table('usranswers', bob)))
        self.action('reportforum', {
                    'user': carol, 'question_id': var['aq'], 'answer_id': var['aq_aa'], 'comment_id': 0}, carol, 'Carol delete alice answer')
        self.assertTrue(compare([], self.table('usranswers', alice)))
        end()
    ...
    def _create_simple_hierarchy(self):
        ...
        self.assertTrue(compare(
            [{'question_id': var['aq'], 'answer_id': var['aq_aa']}], self.table('usranswers', alice)))
        self.assertTrue(compare(
            [{'question_id': var['aq'], 'answer_id': var['aq_ba']}], self.table('usranswers', bob)))
